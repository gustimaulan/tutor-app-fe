...
// Watch for sidebar opening to reset form
...
const resetForm = () => {
  if (isLoggedIn.value) {
    form.value.tutor_name = authStore.user.name
...
const handleSubmit = async () => {
  ...
  try {
    if (!isFormValid.value) {
      formError.value = 'Please fill in all required fields and select an image'
      return
    }
    ...
    try {
      await submitAttendance(form.value)
      emit('submitted')
      closeSidebar()
    } catch (submitError) {
      console.error('Form submission error:', submitError)
      formError.value = submitError.response?.data?.details || submitError.response?.data?.error || submitError.message || 'Failed to submit attendance'
    } finally {
        ...
        <Transition name="slide">
          ...
          >
            ...
            <div class="flex justify-between items-center p-4 sm:p-4 border-b border-gray-200 bg-white sticky top-0 z-10">
              <h3 class="text-lg sm:text-lg font-semibold text-gray-900">Add New Attendance</h3>
              <button 
                @click="closeSidebar" 
                ...
                <div>
                  ...
                  >
                    ...
                    <div v-if="!selectedFile && !isUploading">
                      ...
                      <div class="mt-4">
                        <span class="text-blue-600 font-medium">Choose image</span>
                        <p class="text-gray-500 text-sm mt-1">or drag and drop</p>
                      </div>
                      <p class="text-xs text-gray-400 mt-2">PNG, JPG, GIF, WEBP up to 5MB</p>
            ...
            <div class="border-t border-gray-200 bg-white p-4 sm:p-4 sticky bottom-0">
              ...
              >
                ...
                {{ isCompressing ? 'Compressing...' : isUploading ? 'Uploading...' : isSubmitting ? 'Submitting...' : 'Submit Attendance' }}
              </button>
              </div>
            </div>
          </div>
